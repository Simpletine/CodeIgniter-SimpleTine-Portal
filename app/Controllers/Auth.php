...
class Auth extends BaseController
{
	...
	public function index()
	{
		...
		} else if (!$this->ionAuth->isAdmin()) // remove this elseif if you want to enable this for non-admins
		{
			...
			$this->data['heading'] = lang('Auth.index_heading');
			$this->data['datatables_script'] = true;
			$this->data['scripts'] = [$this->viewsFolder . DIRECTORY_SEPARATOR . 'st-auth-index-script.tpl'];

			// set the flash data error message if there is one
			$this->data['message'] = $this->validation->getErrors() ? $this->validation->listErrors($this->validationListTemplate) : $this->session->getFlashdata('message');
			//list the users
			$authModel = new \App\Models\AuthModel;
			$this->data['users'] = $authModel->getUsers();
			foreach ($this->data['users'] as $k => $user) {
				$this->data['users'][$k]->groups = $authModel->getUsersGroupByUserId($user->id);
			}
			$this->data['contents'] = [$this->viewsFolder . DIRECTORY_SEPARATOR . 'st-auth-index'];
			return $this->st_build();
		}
	}
	...
	public function login()
	{
		...
		if ($this->request->getPost() && $this->validation->withRequest($this->request)->run()) {
			...
			return view('st-includes/st-login', $this->data);
		}
	}
	...
	public function deactivate(int $id = 0)
	{
		if (!$this->ionAuth->loggedIn() || !$this->ionAuth->isAdmin()) {
			// redirect them to the home page because they must be an administrator to view this
			throw new \Exception('You must be an administrator to view this page.');
			// TODO : I think it could be nice to have a dedicated exception like '\Exception\NotAllowed
		}
		...
		if (!$this->validation->withRequest($this->request)->run()) {
			$this->data['heading'] = lang('Auth.deactivate_heading');
			$this->data['css']=['/assets/plugins/icheck-bootstrap/icheck-bootstrap.min.css'];
			$this->data['user'] = $this->ionAuth->user($id)->row();
			$this->data['contents'] = [$this->viewsFolder . DIRECTORY_SEPARATOR . 'st-auth-deactivate-user'];
			return $this->st_build();
		} else {
	...
	{
		$this->data['heading'] = lang('Auth.create_user_heading');
		...
		if ($this->request->getPost() && $this->validation->withRequest($this->request)->run() && $this->ionAuth->register($identity, $password, $email, $additionalData)) {
			...
			$this->data['contents'] = [$this->viewsFolder . DIRECTORY_SEPARATOR . 'st-auth-create-user'];
			return $this->st_build();
		}
	}
	...
	{
		$this->data['heading'] = lang('Auth.edit_user_heading');
		...
		$this->data['contents'] = [$this->viewsFolder . DIRECTORY_SEPARATOR . 'st-auth-edit-user'];
	...
	{
		$this->data['heading'] = lang('Auth.create_group_title');
		...
		if ($this->request->getPost() && $this->validation->withRequest($this->request)->run()) {
			...
			$this->data['contents'] = [$this->viewsFolder . DIRECTORY_SEPARATOR . 'st-auth-create-group'];
			return $this->st_build();
		}
	}
	...
	public function edit_group(int $id = 0)
	{
		...
		$this->data['heading'] = lang('Auth.edit_group_title');
		...
		$this->data['contents'] = [$this->viewsFolder . DIRECTORY_SEPARATOR . 'st-auth-edit-group'];
		return $this->st_build();
	}
